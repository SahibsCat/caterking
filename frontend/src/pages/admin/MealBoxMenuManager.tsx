...
const MealBoxMenuManager = () => {
  ...
  const handleDownloadCSV = () => {
    ...
    const sampleRows = [
      ...
      '"Kids","Premium","Chicken Strips|Fruit Salad|Pasta",1.5\n'
    ].join("");
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-playfair font-bold text-white mb-2">Meal Box Menus</h1>
          <p className="text-gray-400">Configure default dishes for each Meal Box type</p>
        </div>
      ...
      {isBulkAdding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-charcoal p-8 rounded-2xl w-full max-w-md shadow-xl text-center">
            <h2 className="text-2xl font-bold font-playfair mb-6">Bulk Add Meal Box Menus</h2>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-charcoal p-8 rounded-2xl w-full max-w-4xl shadow-xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold font-playfair mb-6">{isEditing ? 'Edit Meal Box Menu' : 'New Meal Box Menu'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-400 mb-1">Box Type</label>
                  <select className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white" value={formData.boxType} onChange={e => setFormData({...formData, boxType: e.target.value as any})}>
                ...
                <div>
                  ...
                  <select className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white" value={formData.package} onChange={e => setFormData({...formData, package: e.target.value as any})}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="bg-tan text-richBlack px-6 py-2 rounded-lg font-bold hover:bg-tan/90">Save Meal Box Group</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            type="text" placeholder="Search box menus..."
      ...
      <div className="bg-charcoal border border-white/10 rounded-2xl overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-white/5 border-b border-white/10">
            <tr>
              <th className="px-6 py-4 font-semibold">Box Type</th>
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">No meal box menus defined</td></tr>
            ) : (
              filteredMenus.map(menu => (
                <tr key={menu._id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4 font-medium">{menu.boxType}</td>
